...
@torch.no_grad()
def run_case(B: int, D: int, H: int, dtype: torch.dtype, dispatch: str):
    device = "cuda"
    torch.manual_seed(0)
...
def main():
    ...
    print(f"\n=== B={args.B} D={args.D} H={args.H} dtype={dtype} dispatch={args.dispatch} ===")
    run_case(args.B, args.D, args.H, dtype, args.dispatch)
