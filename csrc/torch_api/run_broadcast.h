...
run_dual_gemm_broadcast_typed_impl(const at::Tensor &x_in, const at::Tensor &w1_in,
  ...
  TORCH_CHECK(st == cutlass::Status::kSuccess,
              "DualGemm(broadcast B1) run failed");
...
inline std::tuple<at::Tensor, at::Tensor, at::Tensor>
run_dual_gemm_broadcast_typed(const at::Tensor &x_in, const at::Tensor &w1_in,
                    const at::Tensor &w2_in, bool storeD0, bool storeD1) {
  if (storeD0 && storeD1) {
    return run_dual_gemm_broadcast_typed_impl<Element, true, true>(x_in, w1_in, w2_in);
  } else if (storeD0 && !storeD1) {
    return run_dual_gemm_broadcast_typed_impl<Element, true, false>(x_in, w1_in, w2_in);
  } else if (!storeD0 && storeD1) {
    return run_dual_gemm_broadcast_typed_impl<Element, false, true>(x_in, w1_in, w2_in);
  } else {
    return run_dual_gemm_broadcast_typed_impl<Element, false, false>(x_in, w1_in, w2_in);
  }
}
