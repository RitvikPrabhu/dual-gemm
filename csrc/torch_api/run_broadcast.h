...
run_dual_gemm_broadcast_typed_impl(const at::Tensor &x_in,
...
run_dual_gemm_broadcast_typed(const at::Tensor &x_in, const at::Tensor &w1_in,
                              ...
                              bool storeD1) {
  if (storeD0 && storeD1) {
    return run_dual_gemm_broadcast_typed_impl<Element, true, true>(x_in, w1_in,
                                                                   w2_in);
  } else if (storeD0 && !storeD1) {
    return run_dual_gemm_broadcast_typed_impl<Element, true, false>(x_in, w1_in,
                                                                    w2_in);
  } else if (!storeD0 && storeD1) {
    return run_dual_gemm_broadcast_typed_impl<Element, false, true>(x_in, w1_in,
                                                                    w2_in);
  } else {
    return run_dual_gemm_broadcast_typed_impl<Element, false, false>(
        x_in, w1_in, w2_in);
  }
}
